        ...
        <div class="content">
            ...
            <div class="form-box">
                <div class="login-form">
                    ...
                    <input v-model="formData.account" class="the-input mb-[20px]" placeholder="请输入账号" type="text">
                    <input v-model="formData.password" class="the-input mb-[20px]" placeholder="请输入密码" type="password">
                    <button v-ripple :disabled="loading" class="the-btn blue mb-[20px]" style="width: 100%" @click="onLogin(false)">{{ loading ? '登录中...' : '登录' }}</button>
                    <CheckBox v-model="remember" class="mb-[20px]" label="记住账号/密码" />
                    <div v-for="(item, index) in tipList" :key="index" class="tips f-vertical">
                        <button v-copy="item" v-ripple :disabled="loading" class="the-btn mini green">点击复制</button>
                        <div class="tips_text f1">账号: {{ item }}; 密码: 随便填</div>
                        <button v-ripple :disabled="loading" class="the-btn mini blue" @click="setLoginInfo(item)">一键登录</button>
                    </div>
                </div>
            </div>
