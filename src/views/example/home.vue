...
<template>
    <div class="page-home">
        ...
        <div class="mb-[20px]">
            ...
            <a :href="project" class="the-tag green" target="_blank">项目地址</a>
        </div>
        <h2 class="the-title is-line mb-[20px]">布局操作开关</h2>
        <div class="options-box mb-[20px]">
            <CheckBox v-model:value="layoutInfo.showSidebarLogo" class="mb-[20px]" label="显示侧边栏logo"/>
            <CheckBox v-model:value="layoutInfo.showTagList" class="mb-[20px]" label="显示历史记录标签"/>
            <CheckBox v-model:value="layoutInfo.showSidebar" class="mb-[20px]" label="侧边栏展开"/>
            <CheckBox :value="layoutInfo.layoutMode === 'full-header'" label="顶部填满" @change="onMode()"/>
        </div>
        <div v-for="item in settingList" :key="item.label + item.key" class="form-item f-vertical">
            ...
            <template v-else>
                ...
                <template v-if="item.type === 'color'">
                    <input v-model="styleVariable[item.key]" class="the-input" type="text">
                    <div class="color-box">
                        <input v-model="styleVariable[item.key]" class="color-input" type="color" @change="onColor()">
                    </div>
                </template>
                <template v-else>
                    <input v-model="styleVariable[item.key]" class="the-input" type="text" @input="e => onInput(e, item.key as Numbers)">
                    <span class="form-unit">px</span>
                </template>
            </template>
        </div>
        <div class="pt-[20px] mb-[20px]">
            <button v-ripple class="the-btn green mini" @click="copyStyle()">复制当前配置</button>
            <button v-ripple class="the-btn yellow mini" @click="resetStyle()">重置默认样式</button>
            <button v-ripple class="the-btn blue mini" @click="onPreset('green')">应用预设配置-绿色</button>
            <button v-ripple class="the-btn blue mini" @click="onPreset('black')">应用预设配置-黑色</button>
            <button v-ripple class="the-btn blue mini" @click="onPreset('red')">应用预设配置-红色</button>
            <button v-ripple class="the-btn blue mini" @click="onPreset('purple')">应用预设配置-紫色</button>
        </div>
